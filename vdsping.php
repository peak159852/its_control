...
while ($row = mysqli_fetch_array($res)) {
    ...
    if ($imgsrc === "images/reddot.png") {
        echo '<tr id="falping" name="pings" class="falping" ><td width = 200px;> ' . $row['mName'] . '</td>' . '<td>' . $row['mMile'] . 'k</td>' . '<td class="connectview"><img id= "connectview" src=' . $imgsrc . '></td>' . '</tr>';
    } else {
        // echo  '<tr id="allping" name="pings" class="allping"><td width = 200px;>' . $row['mName'] . '</td>' . '<td>' . $row['mMile'] . 'k</td>' . '<td>' . $row['mIP'] . '<td class="connectview"><img id= "connectview" src=' . $imgsrc . '></td>' . '</td></tr>';
    }
