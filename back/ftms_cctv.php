                ...
                <!-- <p class="searchres">검색한 장비 개수 </p> <a class="re-num1"></a> -->
                    <table id="allchart" class="tg">
                        ...
                        <tbody>
                            ...
                            $res = mysqli_query($conn, "SELECT mCameraName,mMile,mEncoderIP FROM ctl_camera_all WHERE mEncoderCorp Like '본선'  ");
                            ...
                            while ($row = mysqli_fetch_array($res)) {
                                ...
                                if ($imgsrc === "images/reddot.png") {
                                    echo '<tr id="falping" name="pings" class="falping" ><td width = 200px;> ' . $row['mCameraName'] . '</td>' . '<td>' . $row['mMile'] . 'k</td>' . '<td>' . $row['mEncoderIP'] . '<td class="connectview"><img id= "connectview" src=' . $imgsrc . '></td>' . '</td></tr>';
                                } else {
                                    echo '<tr id="allping" name="pings" class="allping"><td width = 200px;>' . $row['mCameraName'] . '</td>' . '<td>' . $row['mMile'] . 'k</td>' . '<td>' . $row['mEncoderIP'] . '<td class="connectview"><img id= "connectview" src=' . $imgsrc . '></td>' . '</td></tr>';
                                }
                            }
