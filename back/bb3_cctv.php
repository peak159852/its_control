                ...
                <!-- <p class="searchres">검색한 장비 개수 </p> <a class="re-num1"></a> -->
                    <table id="allchart" class="tg">
                        ...
                        <tbody>
                            ...
                            $res = mysqli_query($conn, "SELECT mCameraName,mMile,mEncoderIP FROM ctl_camera_all WHERE mCameraName REGEXP '북방3|북방2|화촌1터널|화촌2'  ");
                            ...
                            while ($row = mysqli_fetch_array($res)) {
                                ...
                                if ($imgsrc === "images/reddot.png") {
                                    echo '<tr id="falping" class="falping" ><td width = 200px;> ' . $row['mCameraName'] . '</td>' . '<td>' . $row['mMile'] . 'k</td>' . '<td>' . $row['mEncoderIP'] . '<td class="connectview"><img id= "connectview" src=' . $imgsrc . '></td>' . '</td></tr>';
                                } else {
                                    echo '<tr id="allping" class="allping"><td width = 200px;>' . $row['mCameraName'] . '</td>' . '<td>' . $row['mMile'] . 'k</td>' . '<td>' . $row['mEncoderIP'] . '<td class="connectview"><img id= "connectview" src=' . $imgsrc . '></td>' . '</td></tr>';
                                }
                            }
                            ?>
                        </tbody>
